...
public:
	...
	sf::Vector2f spriteCenter(float width, float height) {
		float xPos = position.x + ((width * objectSprite.getScale().x)/2);
		float yPos = position.y + ((height * objectSprite.getScale().y)/2);
